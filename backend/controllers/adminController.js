...
// Shared filter for the Customers report — extends getUsers above (which
// only searches name/email) to also match phone/address/city/state/
// pincode, mirroring what AdminCustomer.jsx's client-side search box
// already checks.
...
//
// FIX: this was originally added to userController.js/userRoutes.js, but
// /api/admin/users is actually served by THIS controller via adminRoutes.js
// (mounted at /api/admin) — userRoutes.js's /admin/users route was dead
// code that never matched any real request, which is why the PDF route
// 404'd even after being "added" there. Moved here to live alongside the
// actual getUsers/createAdminUser/deleteUser handlers that already serve
// /api/admin/users.
//
...
// (matched by order.user) since that data doesn't live on the User model.
